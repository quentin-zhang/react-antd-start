...
/**
 * 将扁平化后的路由信息生成 Route 节点react-router
 *
...
let newredicritpath;
const firstrouter = routerWithReactRouter4.find((routeritem, index) => {
  return index === 0 && routeritem.childRoutes && routeritem.childRoutes.length > 0;
})
if (firstrouter) {
  newredicritpath = firstrouter.path + firstrouter.childRoutes[0].path;
  //const newrouter = <Redirect to={newredicritpath} key={newredicritpath} push />;
  const newrouter =  <Route exact path="/" render={() => <Redirect to={newredicritpath} />}
  />;
  routeChildren.unshift(newrouter);
}
